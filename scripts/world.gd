...
const map_library = preload("res://maps/default_maps.tres")
...
func _ready() -> void:
	menu_manager.host_pressed.connect(host_game)
	menu_manager.join_pressed.connect(join_game)
	menu_manager.exit_lobby_pressed.connect(exit_lobby)
	menu_manager.lobby.start_game_pressed.connect(start_game)
	menu_manager.pause_state_changed.connect(set_pause)
	menu_manager.toggle_music.connect(set_menu_music)
...
func set_menu_music(toggled_on: bool) -> void:
	if !toggled_on:
		menu_music.stop()
	else:
		menu_music.play()
